...
export class Window {
  ...
  createWindows(options: object) {
    ...
    // 创建 electron 窗口的配置参数
    const opt = this.winOpts([args.width || 390, args.height || 590])
    // 判断是否有父窗口
    if (args.parentId) {
    ...
    if (app.isPackaged) {
      ...
      winURL = args.route
        ? `http://${process.env['VUE_APP_SERVER_HOST']}:${process.env['VUE_APP_SERVER_PORT']}${args.route}?winId=${args.id}`
        : `http://${process.env['VUE_DEV_SERVER_HOST']}:${process.env['VUE_DEV_SERVER_PORT']}?winId=${args.id}`
    }
    win.loadURL(winURL)
  ...
  listen() {
    ...
    ipcMain.on('pinUp', (event: Event, winId) => {
      ...
      if (winId && (this.main as BrowserWindow).id === winId) {
        const win: BrowserWindow = this.getWindow(Number((this.main as BrowserWindow).id))
